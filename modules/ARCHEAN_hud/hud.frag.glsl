...
layout(push_constant) uniform PushConstant {
	...
	uvec2 start; // swapchain pixels
	uvec2 size; // draw pixels
	float scale; // from draw pixels to swapchain pixels
};

void main() {
	ivec2 coord = ivec2(round(vec2(gl_FragCoord.xy - start) / scale));
