...
void main() {
	...
	if (surface.renderableData != 0) {
		RenderableData data = RenderableData(surface.renderableData)[nonuniformEXT(surface.geometryIndex)];
		...
		if (data.monitorIndex > 0) {
			surface.emission *= ReverseGamma(texture(nonuniformEXT(textures[nonuniformEXT(data.monitorIndex)]), surface.uv1).rgb);
		}
	}
	if (surface.geometryInfoData > 0) {
		...
		if (tex_albedo > 0) surface.color.rgb *= ReverseGamma(texture(nonuniformEXT(textures[nonuniformEXT(tex_albedo)]), surface.uv1).rgb);
		if (tex_normal > 0) {
			//TODO: normal maps using tex_normal
		}
		if (tex_metallic_roughness > 0) {
			vec2 pbr = texture(nonuniformEXT(textures[nonuniformEXT(tex_metallic_roughness)]), surface.uv1).rg;
		...
		if (tex_emission > 0) {
			...
			surface.emission = emissionPower * ReverseGamma(texture(nonuniformEXT(textures[nonuniformEXT(tex_emission)]), surface.uv1).rgb);
		}
	}
