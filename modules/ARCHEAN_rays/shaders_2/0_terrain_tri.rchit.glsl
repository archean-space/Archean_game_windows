...
void main() {
	...
	if (temperature > 1000) {
		...
		float textureMaxDistanceRatio = pow(clamp(gl_HitTEXT / textureMaxDistance, 0, 1), 0.25);
		float textureNearDistanceRatio = pow(smoothstep(textureNearDistance, textureFarDistance, gl_HitTEXT), 0.25);
		
		// Base terrain
		BUMP(NormalDetail, localPosition * 20, normal, 0.025)
		normal = mix(normal, normal, pow(clamp(gl_HitTEXT / textureNormalMaxDistance, 0, 1), 0.25));
